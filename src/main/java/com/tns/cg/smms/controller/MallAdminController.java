...
import com.tns.cg.smms.service.MallAdminService;
...
//Retrieve operation (retrieving all Admins)
	@GetMapping("/Admins") // used to retrieve resource from server.
	...
	// Create Operation
	@PostMapping("/Admins") // annotation ensures that HTTP POST requests to /Admins are mapped to add()
							// method.
	...
	// Update Operation
	@PutMapping("/Admins/{id}") // It maps the HTTP PUT requests
	public ResponseEntity<?> update(@RequestBody MallAdmin emp, @PathVariable Integer id) {
		...
		try {
			MallAdmin exist = cService.getAdminById(id);
			cService.save(emp);
