...
import com.tns.cg.smms.service.MallService;
...
//Retrieve operation (retrieving all malls)
	@GetMapping("/Malls") // used to retrieve resource from server.
	...
	// Create Operation
	@PostMapping("/Malls") // annotation ensures that HTTP POST requests to /Malls are mapped to add()
								// method.
	...
	// Update Operation
	@PutMapping("/Malls/{id}") // It maps the HTTP PUT requests
	public ResponseEntity<?> update(@RequestBody Mall emp, @PathVariable Integer id) {
		...
		try {
			Mall exist = mService.getMallById(id);
			mService.save(emp);
